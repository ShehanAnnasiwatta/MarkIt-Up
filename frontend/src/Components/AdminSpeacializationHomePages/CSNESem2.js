...
function CSNESem2() {
    ...
    const Datagetting=async()=>{
      ...
      const DeleteItems=(id)=>{
        axios.delete(`http://localhost:3005/AddAssi/CSNESem2Delete/${id}`).then((res)=>{
...
<Stack sx={{width:100,marginLeft:2}}>
                                     <Button variant="outlined" startIcon={<DeleteIcon />} onClick={() => DeleteItems(data._id)}>Delete</Button>
                                        ...
                                        <Button href={`/CSNESem2Add/${i}`} startIcon={<Add />} color="primary"> Add Assignment</Button>
